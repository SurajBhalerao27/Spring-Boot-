...
public class DepartmentController {
	...
	public ResponseEntity<Department> getDept(@PathVariable("did") int did) {
		return new ResponseEntity<>(ds.getDeptDetail(did), HttpStatus.OK);
	}

	// use putmapping to edit existing data
	@PutMapping("/Department/editdept/{did}")
	public ResponseEntity<Department> editDepartment(@PathVariable("did") int did, @RequestBody Department department) {
		return new ResponseEntity<>(ds.updateDeptDetail(department, did), HttpStatus.OK);
	}
}
